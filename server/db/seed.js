...
async function seed() {
  ...
  await santaigoConvo.addUsers([ thomas.id, santiago.id, ])
  ...
  await chiumboConvo.addUsers([chiumbo.id, thomas.id])
  ...
  const hualingGroup = await Conversation.create();

  await hualingGroup.addUsers([hualing.id, thomas.id, chiumbo.id])

  for (let i = 0; i < 11; i++) {
    await Message.create({
      conversationId: hualingGroup.id,
      senderId: hualing.id,
      text: "a test message",
    });
  }
